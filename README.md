...
部署页的每个输入框都提供填写教程和独立验证按钮；Worker 地址会先检查可达性，Bot Token/群组 ID 会调用 Telegram API，其余字段执行格式或范围检查。所有验证按钮、API 验证和生成代码按钮共用同一套本地一次性流程；首次等待一秒后显示“点击跳转”，第二次等待一秒后显示“我已验证”，确认后后续操作不再打断。
